...
- Clamp order is a deliberate, measured difference from the toolkit: we fade then
  clamp, `ForceModel.Compute` clamps then fades. 4 of 640 vector rows differ, all
  at 5-7.5 km/h, worst 1,086/10,000 at the wheel, and only reachable when a force
  past full scale meets a partial fade - a low-speed slide. Recorded as U-2 with
  the table in FORCE-FEEDBACK.md.
