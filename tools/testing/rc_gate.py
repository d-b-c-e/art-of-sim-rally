...
CASES["ffb-lifecycle"] += " For 0.2.6, compare Crash kick off/on at strength 5: one head-on impact and a light side impact, with braking, a landing and restart as negative controls. Assess a distinct kick and smaller fading rebound at fixed mod/Pit House gains; the old RC3 vibration failed owner feel acceptance. Verify no sustained scrape buzz, one bounded cue during landing/crash overlap, and either feature can be disabled without breaking the other. Inspect driver rejection, playback latency/stop reasons and overlap counters; a rejected shape must leave ordinary landing available. Save support while paused. With the optional schema-4 probe, STOP while paused and verify collisions.csv before quitting. Physical crash timing/feel cannot pass from offline tests."
CASES["input-persistence"] += " Verify Crash vibration defaults off when upgrading, and an explicit enable/strength choice survives relaunch."
