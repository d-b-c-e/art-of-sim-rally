...
CASES["ffb-lifecycle"] += " For 0.2.6, compare Crash vibration off/on at strength 5: one head-on impact and a light side impact, with braking, a landing and restart as negative controls. Verify no sustained scrape buzz, one bounded cue during landing/crash overlap, and either feature can be disabled without breaking the other. Check crash driver/overlap counters and save support. With the optional schema-4 probe, STOP while paused and verify collisions.csv before quitting. Physical crash timing/feel cannot pass from offline tests."
